...
func TestGossipService_HandleJoinMessage(t *testing.T) {
	...
	// Track join callback with synchronization
	var mu sync.Mutex
	joinCalled := false
	...
	// Check callback was triggered with lock
	mu.Lock()
	callbackCalled := joinCalled
	peerJoined := joinedPeer
	mu.Unlock()
	...
	if peerJoined == nil || peerJoined.Address.Domain != "bob.collective.local" {
