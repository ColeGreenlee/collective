...
func (c *coordinatorClient) WriteFileStream(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[WriteFileStreamRequest, WriteFileStreamResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Coordinator_ServiceDesc.Streams[0], Coordinator_WriteFileStream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type Coordinator_WriteFileStreamClient = grpc.ClientStreamingClient[WriteFileStreamRequest, WriteFileStreamResponse]

func (c *coordinatorClient) ReadFileStream(ctx context.Context, in *ReadFileStreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ReadFileStreamResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Coordinator_ServiceDesc.Streams[1], Coordinator_ReadFileStream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
func (UnimplementedCoordinatorServer) WriteFileStream(grpc.ClientStreamingServer[WriteFileStreamRequest, WriteFileStreamResponse]) error {
...
func _Coordinator_WriteFileStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(CoordinatorServer).WriteFileStream(&grpc.GenericServerStream[WriteFileStreamRequest, WriteFileStreamResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Coordinator_WriteFileStreamServer = grpc.ClientStreamingServer[WriteFileStreamRequest, WriteFileStreamResponse]

func _Coordinator_ReadFileStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ReadFileStreamRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(CoordinatorServer).ReadFileStream(m, &grpc.GenericServerStream[ReadFileStreamRequest, ReadFileStreamResponse]{ServerStream: stream})
}
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _Coordinator_WriteFileStream_Handler,
			ClientStreams: true,
		},
		{
			StreamName:    "ReadFileStream",
			Handler:       _Coordinator_ReadFileStream_Handler,
			ServerStreams: true,
		},
	},
